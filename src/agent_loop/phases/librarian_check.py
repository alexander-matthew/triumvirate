...
from ..lib.verdicts import AuditVerdict, ParseError
...
def check(pr_number: int) -> int:
    ...
    try:
        with isolated_worktree(
            ...
            try:
                parsed = AuditVerdict.parse(run_.final_message)
            ...
            # Default the notes section if the librarian omitted it.
            notes = parsed.consistency_notes or "(no notes)"
            body = AuditVerdict(verdict=parsed.verdict, consistency_notes=notes).render() + (
